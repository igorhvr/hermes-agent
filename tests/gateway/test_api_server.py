...
class TestResponsesEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_responses_inlines_small_pdf_and_zip_as_data_urls(self, adapter, tmp_path):
        pdf_path = tmp_path / "doc.pdf"
        zip_path = tmp_path / "bundle.zip"
        pdf_path.write_bytes(b"%PDF-1.4\nsmall")
        zip_path.write_bytes(b"PK\x03\x04smallzip")
        mock_result = {
            "final_response": f"MEDIA:{pdf_path}\nMEDIA:{zip_path}",
            "messages": [],
            "api_calls": 1,
        }
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                mock_run.return_value = (mock_result, {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0})
                resp = await cli.post("/v1/responses", json={"input": "attach docs"})
            assert resp.status == 200
            data = await resp.json()
            content_blocks = data["output"][0]["content"]
            file_blocks = [b for b in content_blocks if b.get("type") == "output_file"]
            assert len(file_blocks) == 2
            urls = [b["file_url"] for b in file_blocks]
            assert any(url.startswith("data:application/pdf;base64,") for url in urls)
            assert any(url.startswith("data:application/zip;base64,") for url in urls)
            assert "[File: doc.pdf](data:application/pdf;base64," in content_blocks[0]["text"]
            assert "[File: bundle.zip](data:application/zip;base64," in content_blocks[0]["text"]
